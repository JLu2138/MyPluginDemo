...
public final class ServiceManager {
    ...
    //store intent-conn
    public Map<Object, Intent> mServiceMap2 = new HashMap<>();
    ...
    public int stopService(Intent targetIntent) {
        ...
        if (mServiceMap.isEmpty()) {
            ...
            Context appContext = HostApplication.getContext();
    ...
    private void proxyCreateService(ServiceInfo serviceInfo) throws Exception {
        ...
        serviceInfo.applicationInfo.packageName = HostApplication.getContext().getPackageName();
        ...
        // private void handleCreateService(CreateServiceData data) 方法中会调用 service.onCreate()
    ...
    public void preLoadServices(File apkFile) throws Exception {
        ...
        // 读取 Package 对象里面的 services 字段
        // 接下来要做的就是根据这个 List<Service> 获取到 Service 对应的 ServiceInfo
        List services = (List) RefInvoke.getFieldObject(packageObj, "services");

        // 调用 generateServiceInfo 方法, 把 PackageParser.Service 转换成 ServiceInfo
